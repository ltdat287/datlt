...
class UserController extends Controller
{
    ...
    public function add_conf(UserAddFormRequest $request)
    {
        ...
        $rules = array(
            ...
            'email'              => 'required|max:255|vp_email',
            'email_confirmation' => 'required|vp_same:email',
        ...
        if ($request->has('boss_id')) {
            ...
            if ($boss_id !== '0') {
                // Check boss_id is 0 or id of boss
                if (empty($user)) {
    ...
    {
        // Create rule for validate all input of search form
        $rules = array(
            'name'         => 'max:16',
        ...
        // Create search query to return back browser
        $query_search = '';
        end($rules);
        $last_key_rules = key($rules);

        foreach ($rules as $key => $value)
        {
            if ($key === $last_key_rules) {
                $query_search .= $key . '=';
            } else {
                $query_search .= $key . '=&';
            }
        ...
        if ($validator->fails()) {
            ...
            return redirect('/search?' . $query_search)
            ->withErrors($validator)
            ->withInput();
        }
        ...
        foreach ($roles as $key => $value)
        {
            if (Input::exists($key))
            {
                if (Input::get($key) === '1')
                {
                    ...
                    $errors[] = '権限（' . MemberHelper::getNameRole($value). '）' . trans('に規定値以外の値が入力されました。');
        ...
        // Create display paginate search if has condition
        $display_search = true;
